...
> **런타임에 LLM을 호출하지 않는다.** 방에서 벌어지는 모든 일은 사전 생성된 자산과 순수 로직으로만 돌아간다.
>
...
| **연습 출제 스크립트** | 단어 연상 혼자 모드 | 단어당 3단계 × 400단어 | |
...
### 5.5.2 연습 출제 스크립트 (단어 연상 혼자 모드)

단어 연상은 출제자가 필요해서 원래 혼자 못 한다. 혼자 모드에서는 사람 출제자 대신 이 스크립트를 시간에 따라 흘린다 (03 문서 §7.3). **화면에 가짜 출제자를 세우지 않는다** — 설명은 문제의 일부로 표시된다.
...
- 사람 출제자만큼 재밌진 않다. **혼자서도 이 게임을 맛보게 하는 것**이 목적
