---
name: retro-party
description: retro-party 레포(90년대 소재 실시간 방 파티 게임)에서 코드나 문서를 작성·수정할 때 반드시 먼저 읽는다. 게임방·채팅·팀전·랭크·콘텐츠 파이프라인·UI 어느 쪽이든 해당한다. 어기면 제품이 조용히 깨지는 규약(참가자는 전부 실제 사람, 이모지 금지, 런타임 LLM 호출 금지, 채팅 무저장, 정답 누출 방지, 팀 채널 격리, 컬러 토큰)과 문서 지도를 담고 있다.
---
