...
## 2.55 화면이 반드시 알려줘야 하는 것

빠뜨리기 쉬운데 없으면 게임이 안 읽히는 것들이다. 새 게임을 붙일 때 확인한다.

| | 없으면 |
|---|---|
| **어떻게 하는 게임인가** | 처음 온 사람이 그 라운드를 통째로 버린다. 그건 「사람 있는 방에 들어갔는데 재미없었다」로 남아 03 문서 지표를 갉아먹는다 (`GameGuide`) |
| **몇 라운드째인가** | 판이 언제 끝나는지 몰라 마지막에 힘이 안 들어간다 (`RoundBadge`) |
| **누가 맞혔는가** | 정답만 띄우면 「내가 놓쳤다」가 안 남는다 (정답 공개 화면) |
| **누가 이겼는가** | 판이 끝나는 순간이 [한 판 더] 와 이탈이 갈리는 지점이다. **내가 몇 등인지**를 반드시 (`Podium`) |
| **팀 점수** | 팀전인데 팀 점수가 안 보이면 팀전이 아니다. 개인 점수만으로는 뭉칠 이유가 없다 (`TeamBar`) |
| **왜 점수가 줄었는가** | 조용히 깎이면 이유를 모른다 (채팅의 벌점 배지 + 점수 하강 표시) |
...
**채팅 입력창이 늘 포커스를 잡고 있다.** 정답 입력이 곧 채팅이라 그래야 한다.
그래서 `s` 같은 문자 키를 단축키로 잡으면 그 글자를 못 치고, **한글 IME 에서는
조합 중인 글자가 통째로 깨진다.**

```
쓴다     Alt 조합 · 방향키 · Esc · Enter
안 쓴다   맨 문자 키
반드시    event.isComposing 이면 무조건 무시 (lib/shortcuts.ts)
```

안내는 `hidden lg:flex` 로 넓은 화면에서만 — 폰에는 키보드가 없다.

### 소리 — 파일을 쓰지 않는다

필요한 건 짧은 신호음 대여섯 개뿐이라 Web Audio 로 합성한다.
**파일 0바이트, 라이선스 0, 지연 0.**

```
소리를 낸다   정답 · 라운드 시작 · 정답 공개 · 마지막 5초 · 입장
안 낸다      오답
```

**오답에 소리를 달면 안 된다.** 자주 일어나는 일이라 음소거를 누르게 되고,
그러면 위의 중요한 소리들도 같이 죽는다. 끌 수 없는 소리도 넣지 않는다.

---

## 2.6 모바일과 PC 를 함께 본다

**둘 다 주 사용처다.** 링크로 들어오는 사람은 대부분 폰이고, 오래 노는 사람은 PC 다.
한쪽만 보고 만들면 다른 쪽이 반드시 망가진다.

| | |
|---|---|
| **레이아웃** | 모바일 한 칸(문제 → 사람 → 채팅), `lg:` 부터 두 칸(문제 · 대화). 좁은 화면은 엄지가 닿는 아래에 입력창이 와야 하고, 넓은 화면은 가운데 한 줄만 쓰면 양옆이 논다 |
| **★ 키보드** | **채팅이 정답 입력이라 키보드가 입력창을 덮으면 게임이 안 된다.** `100dvh` 는 주소창만 따라가고 키보드는 못 따라간다 — `env(keyboard-inset-height)` 와 `visualViewport` 로 함께 잡는다 |
| **터치 크기** | 누르는 것은 최소 32~36px. 아이콘만 있는 버튼도 상자를 키운다 |
| **드래그** | 카드 전체를 끌게 하면 모바일에서 페이지 스크롤과 싸운다. **손잡이로만** 끌고, 화살표 대체 조작을 함께 둔다 (연표 정렬) |
| **확대 방지** | 입력 글자는 16px 이상. iOS 사파리는 그 아래면 포커스 시 페이지를 확대한다 |
| **캔버스** | `touch-action: none` 이 없으면 그리는 대신 화면이 스크롤된다 |
